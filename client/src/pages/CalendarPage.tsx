...
import { Plus, Upload, CalendarDays, Loader2, Home, BookOpen, Sparkles, PenLine } from "lucide-react";
...
export default function CalendarPage() {
  ...
  const [blogCategory, setBlogCategory] = useState("메이크업팁");
  const [blogLoading, setBlogLoading] = useState(false);
  const [blogArticles, setBlogArticles] = useState<Array<{ id: number; title: string; slug: string }>>([]);
      ...
      {isAuthenticated && (
        <div className="p-4 border-b bg-muted/30">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold">뷰티 블로그 관리</span>
          </div>
          <div className="flex flex-wrap gap-2 items-center">
            <Button
              ...
              onClick={async () => {
                setBlogLoading(true);
                try {
                  const res = await fetch("/api/cron/generate", { headers: { "x-admin-token": "gn-admin-2025" } });
                  const data = await res.json();
                  toast({ title: `✅ ${data.generated}개 글 자동 생성 완료` });
                  if (data.articles) setBlogArticles(prev => [...data.articles, ...prev]);
                } catch { toast({ title: "생성 실패", variant: "destructive" }); }
                setBlogLoading(false);
              }}
            >
              {blogLoading ? <Loader2 className="h-3 w-3 mr-1 animate-spin" /> : <Sparkles className="h-3 w-3 mr-1" />}
              AI 자동 생성 (2개)
            </Button>
            <input
            ...
            >
              {["기초메이크업","웨딩메이크업","뷰티트렌드","메이크업팁","색조화장","피부관리"].map(c => (
            ...
            <Button
              ...
              variant="outline"
              className="text-xs"
              disabled={blogLoading || !blogKeyword}
              onClick={async () => {
                if (!blogKeyword) return;
                setBlogLoading(true);
                try {
                  const res = await fetch("/api/articles/generate", {
                    method: "POST",
                    headers: { "Content-Type": "application/json", "x-admin-token": "gn-admin-2025" },
                    body: JSON.stringify({ keyword: blogKeyword, category: blogCategory }),
                  });
                  const data = await res.json();
                  toast({ title: `✅ 글 생성 완료: ${data.article?.title}` });
                  setBlogKeyword("");
                  if (data.article) setBlogArticles(prev => [data.article, ...prev]);
                } catch { toast({ title: "생성 실패", variant: "destructive" }); }
                setBlogLoading(false);
              }}
            >
            ...
            <a href="/blog" target="_blank" className="text-xs text-muted-foreground underline ml-1">블로그 보기 →</a>
          </div>
          {blogArticles.length > 0 && (
            <div className="mt-3 space-y-1">
              {blogArticles.slice(0, 5).map(a => (
                <div key={a.id} className="text-xs text-muted-foreground flex items-center gap-2">
                  <span className="text-pink-400">•</span>
                  <a href={`/blog/${a.slug}`} target="_blank" className="hover:underline truncate max-w-xs">{a.title}</a>
                </div>
              ))}
            </div>
          )}
